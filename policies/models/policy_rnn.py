...
class ModelFreeOffPolicy_Separate_RNN(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.kl_optimizer = Adam(self.actor.encode.parameters(), lr=lr)
    ...
    def compute_kl_loss(self, latent_mean, latent_logvar, kl_to_gauss_prior=True):
        # -- KL divergence
    ...
    def forward(self, actions, rewards, observs, dones, masks):
        ...
        kl_loss2 = self.compute_kl_loss(self.critic.latent_mean, self.critic.latent_logvar)
        kl_loss2 = kl_loss2.sum(dim=0).mean()

        self.critic_optimizer.zero_grad()
        (qf1_loss + qf2_loss + kl_loss2).backward()
        ...
        # -----------------
        # kl divergence
        kl_loss1 = self.compute_kl_loss(self.actor.latent_mean, self.actor.latent_logvar)
        kl_loss1 = kl_loss1.sum(dim=0).mean()
        # ------------------------

        kl_coeff = 0.5
        policy_loss += kl_coeff * kl_loss1
    ...
    def report_grad_norm(self):
        ...
        return {
            ...
            "pi_rnn_grad_norm": utl.get_grad_norm(self.actor.encode),
        }
